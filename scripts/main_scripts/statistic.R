...
# calculate the number of mOTUs from phyloseq object as vectors
...
# the dotcharts show the presence of observations with extreme values of both 
# SSS mean and range, deriving from samples that were located in areas 
# (the Black and Baltic seas) naturally showing these values.

# Something similar can be said for both chlorophyll mean and net primary
# productivity mean, but the observed values are not "extremes", rather higher
# then the mean and less "sampled" conditions.

# Salinity transformation:

# in order to test effectively the influence of "extreme" salinity values for 
# those two variables, including conditions of lower salinity mean and higher
# salinity range, these two were log-transformed. However, as salinity mean 
# shows extremes with values lower than the mean, simply applying the logarithm
# is not a valuable option. 
# The distribution was first reflected, the log-transformation applied and the
# reflected again, due to left-skewed nature of the data.

# reflecting the distribution (to all values, the maximum was subtracted and 
# then turned to absolute positive values). 
# A constant of 1 is added.
...
# The effect of the transformation for salinity mean is depicted in this 
# dotchart plot
png("SSS mean transformation process.png",
                             units="cm",
                             width=20,
                             height=30,
                             res=300)
...
# the next plot will compare the relationship between the response variable 
# (OTU richness) and the SSS - related variables before and after the 
# transformation processes.
png("SSS transformation vs number of OTUs.png",
                                    units = "cm",
                                    width = 30,
                                    height = 30,
                                    res = 300)
...
plot.new()
text(.5, .5, "Effect of transformation on the relationship SSS vs #OTUs",
              cex = 1.5,
              font = 2)
...
### check the distribution of the response variable (Not included in the paper)

# here two different methods are implemented: in the first case, the AIC is 
# used to check which distribution has the lower akaike value if fit to the 
# real data, using the fitdistrplus package
...
# data. Here 20 is set, but 5 can work as well
br <- 10
...
png("Response vs explanatory Pearson's rho.png",
                             units = "cm",
                             width = 30,
                             height = 30,
                             res = 300)
...
# the effect of management protection on species richness and diversity is 
# assessed using data from www.protectedplanet.net this is also assessed by 
# looking at the variability
# print Supplementary Material S3.3
png("Protection effect on diversity global.png",
                                    units = "cm",
                                    width = 25,
                                    height = 30,
                                    res = 300)
...
# the same is observed at lower spatial scale, based on the REALM 
# regionalization of Spalding et al. 2007
# print Supplementary Material S3.4
png("Protection effect on richness at the Ecoregion scale.png",
                                          units = "cm",
                                          width = 30,
                                          height = 30,
                                          res = 300)
...
# A high variability can be detected between samples collected at the same
# "site", factoring group created using a buffer resembling the spatial
# resolution of the environmental layers used in the analyses. From the
# following boxplots, it appears evident that the variance in diversity or
# richness is greater for some sites and lower in others. This variability does
# not appear to reflect the latitude per se, rather it appears greater with
# increasing values of OTU richness, possibly due to the type of distribution of
# this data (negative binomial with variance increasing with mean), or simply to
# the methodology, such as DNA metabarcoding, with increasing "uncertainty"
# analyzing samples with a higher number of species.

# here, boxplots are produced on a version of the dataset that has been ordered
# by Latitudinal Zone, showing a trend of increasing variability per site with
# an higher mean of the number of OTUs, rather than the latitude per se.
...
ggsave("Number of OTUs per Ecoregion.png",
                           plot = p1_simp_fin,
                           device = png(),
                           path = NULL,
                           scale = 1,
...
p2 <- ggplot(seas, aes(t, y2)) +
             ...
             ylab("") +
             xlab("Month of year") +
             geom_area(fill = "#00802b", color = "black") +
...
# print Part of Figure 1
ggsave("Schematic Seasonality.png", plot = seasonality,
                                    device = png(),
                                    path = NULL,
                                    scale = 3,
                                    width = 3,
...
# Two subset tables are created, one with the original names and another with
# labels for plots
...
png("Spatial autocorrelation Dataset.png", units = "cm",
                                           width = 30,
                                           height = 20,
                                           res = 300)
plot(corr_spaz)
...
# A tensor product, representing our main hypothesis, will be used as a fixed
# effect

GAMM_norand <- gam(Observed ~ te(nsi, prim_mean),
                   method = "REML",
                   family = nb(theta = NULL,
                               link = "log"),
                               data = tab)

GAMM_rand <- gam(Observed ~ te(nsi, prim_mean) +
                 s(id_site, bs = "re"),
                 method = "REML",
                 family = nb(theta = NULL,
                             link = "log"),
...
png("Spatial Autocorrelation Effect with Random model.png", units = "cm",
                                                            width = 30,
                                                            height = 30,
                                                            res = 300)
...
# Thus, the base model will include a tensor product "an interaction of each
# pair of basis functions for each marginal term, and a penalty for each
# marginal term that penalizes the average wiggliness in that term" between
# primary productivity mean and the normalized seasonality index, and a random
# effect smoother for the site factor

### MODEL SELECTION

# A Generalized Additive Model assuming a negative binomial distribution of the
# response variable is fitted using the environmental variables that contribute
# the least to the collinearity. The Variance Inflating Factor threshold was set
# to 3, and values were calculated using the package "olsrr"

tab_corr_mat <- cor(tab_corr_names)

png("Correlation plot Explanatory Vars.png", units = "cm",
                                             width = 30,
                                             height = 30,
                                             res = 300)
...
png("Response vs uncollinear explanatory Pearson's rho.png", 
                          units = "cm",
                          width = 30,
                          height = 30,
                          res = 300)
...
pear_corr <- round(cor(tab$prim_mean, tab$Observed, method = "pearson"), 2)
plot(tab$prim_mean, tab$Observed, main = pear_corr,
                                  xlab = "Net Primary Productivity mean",
                                  ylab = "#OTUs")
lines(lowess(tab$prim_mean, tab$Observed), col = "red")

pear_corr <- round(cor(tab$l.sal_range, tab$Observed, method = "pearson"), 2)
plot(tab$l.sal_range, tab$Observed, main = pear_corr,
                                    xlab = "Sea Surface Salinity range",
                                    ylab = "#OTUs")
lines(lowess(tab$l.sal_range, tab$Observed), col = "red")

pear_corr <- round(cor(tab$temp_range, tab$Observed, method = "pearson"), 2)
plot(tab$temp_range, tab$Observed, main = pear_corr,
                                   xlab = "Sea Surface Temperature range",
                                   ylab = "#OTUs")
lines(lowess(tab$temp_range, tab$Observed), col = "red")

pear_corr <- round(cor(tab$months, tab$Observed, method = "pearson"), 2)
plot(tab$months, tab$Observed, main = pear_corr,
                               xlab = "Months of deployment",
                               ylab = "#OTUs")
lines(lowess(tab$months, tab$Observed), col = "red")

pear_corr <- round(cor(tab$depth, tab$Observed, method = "pearson"), 2)
plot(tab$depth, tab$Observed, main = pear_corr,
                              xlab = "Depth",
                              ylab = "#OTUs")
...
# those with VIFs lower than 3, can be included in the model. Considering that,
# as the primary productivity can become more avaiable with less seasonality,
# the NSI index will be included in an interaction term with the mean Primary
# Productivity, testing whether with more availability through time, the amount
# of NPP might have an effect

# A final model will be obtained by the backword selection process, and another
# model will be computed including exclusively the tensor product between the
# NSI and NPP mean
...
GAMM0 <- gam(Observed ~ te(nsi, prim_mean) +
                        s(months) +
                        s(depth) +
                        s(temp_range) +
                        s(l.sal_range) +
...
GAMM1 <- gam(Observed ~ te(nsi, prim_mean) +
                        s(temp_range) +
                        s(l.sal_range) +
                        s(id_site, bs = "re"),
                                   method = "REML",
...
# print Supplementary Material S3.7
png("NSI-NPP mean Physical extremes.png", units = "cm",
                                          width = 20,
                                          height = 20,
                                          res = 300)
gam.check(GAMM1)
...
# print Supplementary Material S3.8
png("NSI-NPP mean Physical extremes_smoothers.png", units = "cm",
                                                    width = 20,
                                                    height = 20,
                                                    res = 300)
draw(GAMM1)
...
GAMM2 <- gam(Observed ~ te(nsi, prim_mean) +
                        s(id_site, bs = "re"),
                                   method = "REML",
                                   family = nb(theta = NULL,
                                               link = "log"),
...
vis.gam(GAMM0, type = "response",
               plot.type = "contour",
               main = "NSI*NPP mean + All non-collinear",
               xlab = "",
               ylab = "NPP mean")
vis.gam(GAMM1, type = "response",
               plot.type = "contour",
               main = "NSI*NPP mean + Physical extremes",
               xlab = "NSI",
               ylab = "",
               yaxt = "n")
vis.gam(GAMM2, type = "response",
               plot.type = "contour",
               main = "NSI*NPP mean",
               xlab = "",
               ylab = "",
...
mod_res <- as.data.frame(cbind(c("nsi", 
                                 "chl_mean",
                                 "prim_mean",
                                 "temp_mean"), c("NSI",
                                                 "CHL mean",
...
# print Figure 3
png("Alternative GAMs smoothers.png", units = "cm",
                                      width = 20,
                                      height = 20,
                                      res = 300)
op <- par(mfrow = c(2, 2))
...
for (i in seq(1, nrow(mod_res))) {
    ...
    formula <- substitute(Observed ~ s(x) + s(id_site, bs = "re"),
...
# Cross-Validation on different models with SST mean, NPP mean, CHL mean, NSI 
# and the GAMM3 and GAMM1 from the previous model selection steps. Due to the
# high number of explanatory variables in the model GAMM1 (here GAMMd), as the 
# dataset include a "limited" number of observations, the degrees of freedom 
# were limited to 8 for each spline smoother (for SSS and SST)

# set number of trials for random subsetting of the dataset 
ntrial <- 100

# create the dataframe storing the RMSE values

res <- data.frame(matrix(0, ncol = ntrial, nrow = 6),
                  row.names = c("SST mean",
                                "NSI",
                                "Tensor product \n NSI and NPP mean",
                                "Full Model",
                                "CHL mean",
...
# create lists to store results from each model in each trial
moda <- list()
modb <- list()
modc <- list()
modd <- list()
...
GAMMa <- gam(Observed ~ s(temp_mean) + s(id_site, bs = "re"),
             ...
             family = nb(theta = NULL,
                         link = "log"),
             data = training)
GAMMb <- gam(Observed ~ s(nsi) + s(id_site, bs = "re"),
             method = "REML",
             family = nb(theta = NULL,
                         link = "log"),
             data = training)
GAMMc <- gam(Observed ~ te(nsi,prim_mean) + s(id_site, bs = "re"),
             method = "REML",
             family = nb(theta = NULL,
                         link = "log"),
             data = training)
GAMMd <- gam(Observed ~ te(nsi,prim_mean) + s(temp_range, k = 8) +
                                            s(l.sal_range, k = 8) +
                                            s(id_site, bs = "re"),
                           method = "REML", family = nb(theta = NULL,
                                                        link = "log"),
                           data = training)
GAMMe <- gam(Observed ~ s(chl_mean) + s(id_site, bs = "re"),
             method = "REML",
             family = nb(theta = NULL,
                         link = "log"),
             data = training)
GAMMf <- gam(Observed ~ s(prim_mean) + s(id_site, bs = "re"),
             method = "REML",
             family = nb(theta = NULL,
                         link = "log"),
             data = training)
...
catPreda <- c(predict.gam(GAMMa, 
                       newdata = validation1[, "temp_mean", drop = FALSE],
                       exclude = 's(id_site)',
                       newdata.guaranteed = TRUE,
                       se.fit = TRUE,
                       type = c("response")))
catPredb <- c(predict.gam(GAMMb, newdata = validation1[, "nsi", drop = FALSE],
                       exclude = 's(id_site)',
                       newdata.guaranteed = TRUE,
                       se.fit = TRUE,
                       type=c("response")))
catPredc <- c(predict.gam(GAMMc, newdata = validation1[, c("nsi", "prim_mean")],
                       exclude = 's(id_site)',
                       newdata.guaranteed = TRUE,
                       se.fit = TRUE,
                       type = c("response")))
catPredd <- c(predict.gam(GAMMd, newdata = validation1[, c("nsi",
                                                        "prim_mean",
                                                        "temp_range",
                                                        "l.sal_range")],
                       exclude = 's(id_site)',
                       ...
                       se.fit = TRUE,
                       type = c("response")))
catPrede <- c(predict.gam(GAMMe, newdata = validation1[, "chl_mean",drop = FALSE],
                       exclude = 's(id_site)',
                       newdata.guaranteed = TRUE,
                       se.fit = TRUE,
                       type = c("response")))
catPredf <- c(predict.gam(GAMMf, newdata = validation1[, "prim_mean", drop = FALSE],
                       exclude = 's(id_site)',
                       newdata.guaranteed = TRUE,
                       se.fit = TRUE,
                       type = c("response")))
...
resPata <- data.frame(catPreda$fit, catPreda$se.fit,
                      validation1[, "temp_mean", drop = FALSE])
resPatb <- data.frame(catPredb$fit, catPredb$se.fit,
                      validation1[, "nsi", drop = FALSE])
resPatc <- data.frame(catPredc$fit, catPredc$se.fit,
                      validation1[, c("nsi", "prim_mean")])
resPatd <- data.frame(catPredd$fit, catPredd$se.fit,
                      validation1[, c("nsi", "prim_mean", "temp_range", "l.sal_range")])
resPate <- data.frame(catPrede$fit, catPrede$se.fit,
                      validation1[, "chl_mean", drop = FALSE])
resPatf <- data.frame(catPredf$fit, catPredf$se.fit,
                      validation1[, "prim_mean", drop = FALSE])
...
for (j in seq(.4, .8, by = .1)) {

# create "copy" of the validation dataset
validation2 <- validation1

# fix the values for the seasonality index of the validation dataset to the
# values chosen by the loop arguments (from 0.4 to 0.8). These "moderate" values
# were chosen to restrain the prediction of the model to less extreme values,
# not necessarily more represented
validation2$nsi <- j

catPredc_nsifix <- c(predict.gam(GAMMc, newdata = validation2[, c("nsi", "prim_mean")],
                                exclude = 's(id_site)',
                                newdata.guaranteed = TRUE,
                                se.fit = TRUE,
                                type = c("response")))
catPredd_nsifix <- c(predict.gam(GAMMd, newdata = validation2[, c("nsi", "prim_mean", "temp_range", "l.sal_range")],
                                exclude = c('s(id_site)', 's(l.sal_range)', 's(temp_range)'),
                                newdata.guaranteed = TRUE,
                                se.fit = TRUE,
                                type = c("response")))

resPatc_fix <- data.frame(catPredc_nsifix$fit, catPredc_nsifix$se.fit,
                          validation2[, c("nsi", "prim_mean")])
resPatd_fix <- data.frame(catPredd_nsifix$fit, catPredd_nsifix$se.fit,
                          validation2[,c("nsi", "prim_mean", "temp_range", "l.sal_range")])
...
violins <- ggplot(res2_ord) +
                  ...
                  xlab("Fixed effects") +
                  ylab("RMSE") +
                  geom_boxplot(aes(col, value), width = .05) +
